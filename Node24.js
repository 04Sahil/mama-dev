...
Use real aptitude style, medium difficulty.
    ...
    // Expose the analyzer functions to window for testing
    window._udemyAnalyzer = {
...
(function() {
  ...
  testPanel.style.cssText = 'position:fixed;top:20px;right:20px;background:#fff;padding:10px;z-index:99999;border:1px solid #ccc;font-family:sans-serif;';
  
  testPanel.innerHTML = `
    <h4 style="margin:0 0 10px 0">XP System Test</h4>
    <div style="display:grid;gap:5px;">
      <button id="testAdd50">+50 XP</button>
      <button id="testAdd100">+100 XP</button>
      <button id="testAdd250">+250 XP</button>
      <button id="testReset">Reset XP</button>
    </div>
    <div id="xpDebug" style="margin-top:10px;font-size:12px;"></div>
  `;
  
  document.body.appendChild(testPanel);
  
  document.getElementById('testAdd50').addEventListener('click', () => {
    window._udemyAnalyzer.addXP(50);
    updateDebugInfo();
  });
  
  document.getElementById('testAdd100').addEventListener('click', () => {
    window._udemyAnalyzer.addXP(100);
    updateDebugInfo();
  });
  
  document.getElementById('testAdd250').addEventListener('click', () => {
    window._udemyAnalyzer.addXP(250);
    updateDebugInfo();
  });
  
  document.getElementById('testReset').addEventListener('click', () => {
    localStorage.removeItem('udemyXPData_xp');
    localStorage.removeItem('udemyXPData_lvl');
    location.reload();
  });
  
  function updateDebugInfo() {
    const xp = localStorage.getItem('udemyXPData_xp') || 0;
    const lvl = localStorage.getItem('udemyXPData_lvl') || 1;
    document.getElementById('xpDebug').innerHTML = `
      Current: ${xp} XP<br>
      Level: ${lvl}
    `;
  }
