...
(function () {
    ...
    btn.onclick = async () => {
        ...
        try {
            ...
            // Remove previous chatCard if any
            const oldChat = document.getElementById('udemyChatFooter');
            if (oldChat) oldChat.remove();

            // 2. Chat Interface (sticky at bottom INSIDE udemyAnalysisPanel)
            const chatCard = document.createElement('div');
            chatCard.id = 'udemyChatFooter';
            ...
            // Inner layout
            chatCard.innerHTML = `
                <h4 style="margin:0 0 8px 0;">💬 Ask About This Course</h4>
                <div style="display:flex;gap:8px;align-items:flex-end;">
                    <textarea class="udemy-input" placeholder="Type your question here..." rows="2" style="resize:none;flex:1;"></textarea>
                    <button class="udemy-tool-btn secondary" id="askBtn" style="margin:0;">Ask</button>
                </div>
                <div id="chatResponse" style="margin-top:10px;min-height:24px;"></div>
            `;

            // Append as LAST CHILD of panel
            panel.appendChild(chatCard);

            // Make sure panel has enough bottom padding so content is not hidden behind chatCard
            panel.style.paddingBottom = chatCard.offsetHeight + 24 + 'px';
            ...
            askBtn.onclick = async () => {
                ...
                reply.innerHTML = `
                    <div style="display:flex;align-items:center;">
                        <div class="loader" style="border:3px solid #f3f3f3;border-top:3px solid ${styles.secondaryColor};width:20px;height:20px;border-radius:50%;animation:spin 1s linear infinite;"></div>
