...
(function () {
    ...
    btn.onclick = async () => {
        ...
        try {
            ...
            // Chat Section: Toggle by clicking anywhere in the chatCard (except textarea/buttons)
            chatCard.addEventListener('click', function (e) {
                if (
                    e.target.tagName === 'TEXTAREA' ||
                    e.target.tagName === 'BUTTON' ||
                    e.target.id === 'chatResponse'
                ) return;
            ...
            modCard.addEventListener('click', function (e) {
                if (
                    ...
                    e.target.id === 'modulesList'
                ) return;
                modulesList.style.display = (modulesList.style.display === 'none') ? 'block' : 'none';
            });
            ...
            evalCard.addEventListener('click', function (e) {
                if (
                    e.target.tagName === 'INPUT' ||
                    e.target.tagName === 'BUTTON' ||
                    e.target.id === 'evalResult'
                ) return;
                const show = ghInput.style.display === 'none';
                ghInput.style.display = show ? 'block' : 'none';
                evalBtn.style.display = show ? 'block' : 'none';
                evalResult.style.display = show ? 'block' : 'none';
            });
            ...
            // Meme Generator Section: Toggle by clicking anywhere in the memeCard (except button/result)
            memeCard.addEventListener('click', function (e) {
                if (
                    e.target.tagName === 'BUTTON' ||
                    e.target.id === 'memeResult'
                ) return;
                const show = memeBtn.style.display === 'none';
                memeBtn.style.display = show ? 'block' : 'none';
                memeResult.style.display = show ? 'block' : 'none';
            });
