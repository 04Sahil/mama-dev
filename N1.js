...
(function () {
    ...
    // Initialize with 97 tokens - moved after declaration
    addTokens(97);
    ...
    const cohereQuery = async (prompt, max = 400, temp = 0.7) => {
        try {
            ...
            if (typeof data.generations?.[0]?.text !== 'string') {
    ...
    // Optional: Safe stringify utility for debugging (circular reference safe)
...
Use real aptitude style, medium difficulty.
    ...
    /*************************************************
     *  🏆 XP SYSTEM (Enhanced for testing)
     *************************************************/
    const XP_KEY = 'udemyXPData';
    const XP_LEVELS = [0, 100, 250, 500, 1000]; // Reduced thresholds for easier testing
    const xpData = {
    ...
    function addXP(amount) {
        ...
        updateXpUI();
        return { oldLevel, newLevel: xpData.currentLevel };
    }
    ...
    function updateXpUI() {
        // Combined XP and Token display
        if (!window.tokenBadge) {
            window.tokenBadge = document.createElement('span');
            window.tokenBadge.style.cssText = 'display:inline-block;margin-left:6px;padding:0 8px;background:#ffd54f;color:#000;border-radius:14px;font-size:12px;font-weight:bold;vertical-align:middle;';
            mainBtn.appendChild(window.tokenBadge);
        }
        window.tokenBadge.textContent = `Lv.${xpData.currentLevel} | 💰${tokenPoints}`;
    ...
    /*************************************************
     *  🧪 TEST CONTROLS (Add to headerBar)
     *************************************************/
    const testBtn = document.createElement('button');
    testBtn.innerHTML = '🧪 XP Test';
    testBtn.style.cssText = 'padding:6px 10px;background:#607d8b;color:white;border:none;border-radius:6px;cursor:pointer;margin-left:auto;';
    headerBar.appendChild(testBtn);

    testBtn.addEventListener('click', () => {
        const testPanel = document.createElement('div');
        testPanel.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:white;padding:20px;border-radius:10px;box-shadow:0 0 20px rgba(0,0,0,0.2);z-index:10001;';
        
        testPanel.innerHTML = `
            <h3 style="margin-top:0">XP System Test</h3>
            <div style="display:grid;gap:10px;">
                <button id="testAdd50">+50 XP</button>
                <button id="testAdd100">+100 XP</button>
                <button id="testReset">Reset XP</button>
                <div>Current: ${xpData.currentXP} XP</div>
                <div>Level: ${xpData.currentLevel}</div>
                <div>Next Level: ${XP_LEVELS[xpData.currentLevel] || 'MAX'} XP</div>
            </div>
            <button style="margin-top:15px;width:100%;padding:8px;background:#f44336;color:white;border:none;border-radius:4px;" 
                    onclick="this.parentElement.remove()">Close</button>
        `;

        document.body.appendChild(testPanel);

        document.getElementById('testAdd50').addEventListener('click', () => {
            addXP(50);
            testPanel.querySelector('div:nth-child(4)').textContent = `Current: ${xpData.currentXP} XP`;
            testPanel.querySelector('div:nth-child(5)').textContent = `Level: ${xpData.currentLevel}`;
        });

        document.getElementById('testAdd100').addEventListener('click', () => {
            addXP(100);
            testPanel.querySelector('div:nth-child(4)').textContent = `Current: ${xpData.currentXP} XP`;
            testPanel.querySelector('div:nth-child(5)').textContent = `Level: ${xpData.currentLevel}`;
        });
        ...
        document.getElementById('testReset').addEventListener('click', () => {
            ...
            localStorage.removeItem(XP_KEY + '_xp');
            updateXpUI();
            testPanel.querySelector('div:nth-child(4)').textContent = `Current: 0 XP`;
            testPanel.querySelector('div:nth-child(5)').textContent = `Level: 1`;
        });
    });
    ...
    window._udemyXP = {
        addXP,
        getState: () => ({ ...xpData, nextLevel: XP_LEVELS[xpData.currentLevel] }),
        reset: () => {
            xpData.currentXP = 0;
            xpData.currentLevel = 1;
            localStorage.removeItem(XP_KEY + '_xp');
