...
/* --- PUZZLE GENERATOR ------------------------------------ */
    ...
    puzzleBtn.onclick = async () => {
        ...
        try {
            ...
            const clues = {};
            cluesText.split('\n').forEach(line => {
        // Use first ':' or '-' as separator, ignore empty lines
        if (!line.trim()) return;
        const [module, clue] = line.split(/[:\-]/).map(part => part.trim());
        if (module && clue) {
            clues[module.toUpperCase().replace(/\s+/g, '')] = clue;
        }
    });

            // Create puzzle words from module names
            const words = chosen.map(module => ({
                text: module.toUpperCase().replace(/\s+/g, ''),
                clue: clues[module.toUpperCase().replace(/\s+/g, '')] || `Related to ${module}`
            })).filter(word => word.text.length <= 10); // Filter words that are too long
    ...
    function generatePuzzleGrid(words) {
        // Dynamic grid size: longest word + 4, at least words.length + 4, min 10, max 18
        const maxWordLen = Math.max(...words.map(w => w.text.length), 0);
        const GRID_SIZE = Math.max(10, Math.min(18, Math.max(maxWordLen + 4, words.length + 4)));
        ...
        for (let row = 0; row < GRID_SIZE; row++) {
            for (let col = 0; col < GRID_SIZE; col++) {
                ...
                if (cellData === null) {
                    ...
                    letter.style.cssText = 'display:flex;align-items:center;justify-content:center;height:100%;font-weight:bold;';
